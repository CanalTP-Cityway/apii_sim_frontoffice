 ...
 */
angular.module('apiiSimFrontofficeApp').service('Sim',
		[ '$rootScope', '$location', '$q', '$log', '$timeout', function($rootScope, $location, $q, $log, $timeout) {
			...
			this.send = function(value) {
				...
				self.clear();
				 ...
				 this.url = 'ws://' + $location.host() + ':' + $location.port() + '/planner';
			...
			this.opening = function(value) {
				...
				this.socket.onmessage = function(message) {
					$rootScope.$apply(function() {
						...
						$log.info('[DSU] receive : ' + JSON.stringify(value));
						var type = Object.keys(value)[0];
						switch (type) {
						case 'PlanTripResponse':
							if (value.PlanTripResponse.Status == 0) {
								self.context.onPlanTripResponseValid(value);
							} else {
								self.context.onPlanTripResponseInvalid(value);
							}
							true
							break;
						case 'AbstractResponseType':
							self.context.onPlanTripCancellationResponse(value);
							break;
						case 'StartingSearch':
							self.context.onStartingSearch(value);
							break;
						case 'PlanTripExistenceNotificationResponseType':
							self.context.onPlanTripExistenceNotificationResponse(value);
							break;
						case 'PlanTripNotificationResponseType':
							self.context.onPlanTripNotificationResponse(value);
							break;
						case 'EndingSearch':
							self.context.onEndingSearch(value);
							break;
						default:
							break;
						}
			...
			this.sendPlanTripRequest = function(value) {
				...
				if (this.socket !== null) {
					$log.warn('[DSU] send to ' + this.url + ' message : ' + JSON.stringify(value));
			...
			this.receivePlanTripResponseInvalid = function(value) {
				$rootScope.model.plantrip.response = value;
			};

			this.sendPlanTripCancellationRequest = function(value) {
			...
			this.receivePlanTripCancellationResponse = function(value) {
			...
			this.receivePlanTripExistenceNotificationResponse = function(value) {
				var entry = {
					header : value,
					body : null
				};
			...
			this.receivePlanTripNotificationResponse = function(value) {
